      ...
      cocktails.forEach((cocktail, index) => {
        ...
        carouselLink.addEventListener('click', (event) => {
          ...
          const buttonHTML = '<button id="myButton">Guardar en favoritos</button>' +
            '<button id="deleteButton">Eliminar de favoritos</button>';
          ingredientsWindow.document.querySelector('p').insertAdjacentHTML('afterend', buttonHTML);

          // Agregar un evento click al botón para guardar la receta de favoritos
          ...
          // Agregar un evento click al botón para eliminar la receta de favoritos
          ingredientsWindow.document.getElementById('deleteButton').addEventListener('click', () => {
            // Eliminar los ingredientes del Local Storage
            localStorage.removeItem(shortenedName);
            alert('Has eliminado la receta de favoritos');
          });
        });
